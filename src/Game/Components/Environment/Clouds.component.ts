...
class Data {
  ...
  constructor(
    ...
    const uniforms = ["world", "viewProjection", "cameraPosition", "offset"];
    ...
    if (renderer.sceneOptions.ubo.buffer) {
      ...
      renderer.sceneOptions.ubo.observers.beforeSync.add(() => {
        renderer.sceneOptions.ubo.syncToShaderMaterial(false, cloudMaterial);
      });
    }
...
void main(void) {
  ...
  p.x -= offset;
  vWorldPOS = vec3( vec4(finalWorld * p).xyz);
