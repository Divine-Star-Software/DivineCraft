...
import { Controls, KeyDownEvent, PointerDownEvent } from "@amodx/controls";
...
class Data {
  ...
  middle = new Observable<MouseEvent>();
  middleReleased = new Observable<MouseEvent>();
  numberPressed = new Observable<number>();
...
export const PlayerControlsComponent = NCS.registerComponent<Data>({
  ...
  schema: NCS.schema({
    hotBarEnabled: NCS.property(true),
    menuEnabled: NCS.property(true),
    uiOnly: NCS.property(false),
  }),
  init(component) {
    const {
      data: { engine },
    ...
    const numberListener = ({ key }: KeyboardEvent) => {
      if (key >= "0" && key <= "9") {
        controls.numberPressed.notify(parseInt(key));
      }
    };
    window.addEventListener("keydown", numberListener);
    ...
    Controls.registerControls([
      {
        ...
        controls: [
          ...
          },
          {
            ...
            action(event: PointerDownEvent) {
              if (component.schema.uiOnly) return;
              if (controls.menuOpen) return;
              controls.middle.notify(event.origin);
              event.addEventListener("release", () => {
                controls.middleReleased.notify(event.origin);
              });
            },
          },
          {
    ...
    controls.dispose = () => {
      console.warn("DIPOSE CONTROLS");
      Controls.deinit();
      window.removeEventListener("keydown", numberListener);
    };
  },
  dispose(component) {
